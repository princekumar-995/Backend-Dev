...
export const createEmployee = (req, res) => {
  ...
  let { name, email, department, basicSalary, gender, startDate, salary } =
    req.body;
  ...
  if (!name || !email || !department || !basicSalary || !gender) {
    // Render the form again with an error message
    return res
      .status(400)
      .render("employeeForm", { error: "All fields are required." });
  }
  const newEmployee = {
    id: Date.now(),
    name,
...
export const updateEmployee = (req, res) => {
  ...
  let employee = employeeData.find(
    (emp) => emp.id.toString() === id || emp.id === Number(id),
  );
  if (!employee) {
  ...
  // Support both JSON and form submissions
  const { name, email, department, basicSalary, gender, startDate, salary } =
    req.body;
